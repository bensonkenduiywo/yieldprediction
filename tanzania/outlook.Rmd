...
tmap_mode("view")
map_b <- tm_shape(tza.1, name="Average yield (MT/ha) per Region") +
...
Now lets see how  *season 3* looks spatially

```{r v6, echo=F , message=F}
library(tmap)
library(mapview)
tza.1  <- tza
tza.1 <- subset(tza.1, date==year & Season=="LR")
tmap_mode("view")
map_b <- tm_shape(tza.1, name="Average yield (MT/ha) per Region") +
  tm_polygons(col=c("gwad", "lai"),  n=5,  title=c(paste(year, " Yield (MT/ha)"), "Leaf Area Index"),n=5) +
  tm_facets(sync = TRUE, ncol = 2, nrow=1) +
