...
tt <- read.csv(paste0(root, "RHEAS/tanzania_tamsat_25km_districts_LR_dssatTable_2000_2022_100kg.csv"), stringsAsFactors =  FALSE)
...
#rh <- RH_metrics(tt, sStart ="02", sEnd = "09", "MAM")
...
library(raster)
#Long Rain season
filename <- "D:/Adm data/Tanzania/gadm36_TZA_2_LongRain.shp"
...
year <- 2022
temp <- lr[, c("Region", "date", "gwad", "lai")]
...
map_a <- tm_shape(tza, name="Average yield (MT/ha) per Region") +
  tm_fill(col=c("gwad", "lai"),   title=c("Yield (MT/ha)", "LAI"), n = 5) +
  tm_facets(sync = TRUE, ncol = 2, nrow=2) +
  #tm_text("District", size = 0.75)+
  tm_layout(title.size =5, panel.label.size=6)+
  tm_format("World")
