...
```{r v1}
val <- data.frame()
for(i in 1:length(unique(dff$cultivar))){
  print(paste0("cultivar",i))
  temp <- subset(dff, cultivar==paste0("cultivar",i))
  ...
  output <- c(i, rmse_m, rrmse_m, rmse25, rrmse25, rmse50, rrmse50, rmse75, rrmse75, rmse95, rrmse95)
  val <-  rbind(val, output)
}
colnames(val) <-c("Cultivar", "RMSE_mean", "RRMSE_mean", "RMSE25", "RRMSE25", "RMSE50", "RRMSE50", "RMSE75", "RRMSE75", "RMSE95", "RRMSE95")

knitr::kable(val,caption = "Cultivar assessement based on ensemble mean, 25%. 50%, 75% and 95% percentiles respectively.")
```


Cultivars 6 and 7 seem to show lower RMSE and RRMSE especially on mean of RHEAS ensemble yield forecasts. These cultivars corresponds to the following names. So cultivar 6 (DKC8033) seem to be give better generaliation of yields both in Malawi and Zambia.
